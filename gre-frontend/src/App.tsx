...
// 引入图表库 (需安装: npm install recharts)
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, AreaChart, Area } from 'recharts';

// ================== 配置区域 ==================
// 关键修改: 优先从环境变量读取后端地址。如果没有(比如本地开发)，则默认使用 localhost。
// 在 Vercel 部署时，我们需要在后台设置 VITE_API_URL 这个变量
...
const MOCK_QUESTIONS = [
  {
    id: 'mock-1',
    type: '单选题',
    question: "[本地演示模式] 如果 x > 0 且 x² - 4x - 12 = 0，那么 x 的值是多少？\n(注意：当前未连接到后端数据库)",
    options: ["2", "4", "6", "8", "12"],
    correctAnswer: 2, 
    explanation: "这是本地兜底数据。请检查后端服务是否启动。"
  }
];
...
// ================== 2. 工具函数 ==================
// 将后端 MongoDB 的数据格式，转换为前端 UI 需要的格式
const adaptBackendData = (backendQuestions) => {
  if (!Array.isArray(backendQuestions)) return [];
  
  return backendQuestions.map((q) => {
    // 提取选项文本
    const optionsText = q.options ? q.options.map(opt => opt.text || opt.content || String(opt)) : [];
...
const GREModule = () => {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        setQuestions(adaptBackendData(data));
        setUseRealData(true);
      } else {
        setQuestions(MOCK_QUESTIONS);
        setUseRealData(false);
        setErrorMsg('后端数据库为空，请先录题');
      }
    } catch (error) {
      console.error("Fetch Error:", error);
      setQuestions(MOCK_QUESTIONS);
      setUseRealData(false);
      setErrorMsg('连接失败 (请检查后端是否启动)');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (isExamFinished) {
    return (
      <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 text-center max-w-2xl mx-auto mt-10">
        <Award size={64} className="mx-auto text-yellow-500 mb-4" />
        <h2 className="text-3xl font-bold text-slate-800 mb-2">练习完成</h2>
        <p className="text-5xl font-bold text-blue-600 mb-8">{score} / {questions.length}</p>
        <button onClick={() => window.location.reload()} className="bg-blue-600 text-white px-8 py-3 rounded-lg hover:bg-blue-700">再练一次</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      {/* 状态提示条 */}
      <div className={`text-xs px-4 py-2 mb-4 rounded flex justify-between items-center ${useRealData ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
        <div className="flex items-center">
          {useRealData 
            ? <span>✅ 已连接真实后端数据库 (共 {questions.length} 题)</span>
            : <span>⚠️ {errorMsg || '使用本地模拟数据'}</span>
          }
        </div>
        {!useRealData && (
            <button onClick={fetchQuestions} className="ml-4 font-bold hover:text-yellow-900 flex items-center">
              <RefreshCw size={12} className="mr-1"/> 重试连接
            </button>
        )}
      </div>
